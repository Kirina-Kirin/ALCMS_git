...
if($values){
  ...
  $out .= '<div><ul class="barchart" style="width:'.$width.'px;height:'.($height).'px;">';
  foreach($values as $v){
    $out .= '<li style="width:'.$barwidth.'px; display: table;border-top-width:'.floor($height-($v*$factor)).'px;';
    if($bargap > 0){
      $out .= 'margin-left:'.($bargap/2).'px;margin-right:'.($bargap/2).'px;';
    }        
    $out .= '"><span>'.$v.'</span></li>';
  } 
  $out .= '</ul>';
